...
def main(file_path, file_uuid):
    ...
    # Characterization always occurs - if nothing is specified,
    # get one or more defaults specified in the FPR.
    if not rules:
    ...
    for rule in rules:
        if rule.command.script_type == 'bashScript' or rule.command.script_type == 'command':
            args = []
            command_to_execute = rule.command.command.replace('%relativeLocation%',
                file_path)
            command_to_execute = command_to_execute.replace('%fileUUID%',
                file_uuid)
        else:
            args = [file_path, file_uuid]
            command_to_execute = rule.command.command

        exitstatus, stdout, stderr = executeOrRun(rule.command.script_type, command_to_execute, arguments=args)
        if exitstatus != 0:
